...
def _build_gemini(settings: Settings) -> GeminiAgentClient:
...
def build_agent_client(settings: Settings) -> AgentClient:
    """Выбирает Mock / Gemini / OpenRouter; Gemini+ключ OpenRouter → failover."""
    ...
    if impl == "gemini":
        ...
        if openrouter_key:
            return FailoverAgentClient(
                primary=primary,
                fallback=_build_openrouter(settings),
                primary_name="gemini",
    ...
    raise AgentConfigError(
        ...
        "Ожидается 'mock', 'gemini' или 'openrouter'."
    )
